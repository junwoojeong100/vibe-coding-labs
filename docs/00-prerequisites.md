...
실습을 시작하기 전에 아래 계정·도구·권한을 준비합니다. 각 항목 끝의 **확인 명령**으로 설치가 정상인지 검증하세요.
...
> 💡 역할 이름이 최근 변경되었습니다. **Foundry Project Manager**는 이전의 *Azure AI Project Manager*와 동일합니다.
...
설치 후 **Foundry agents 확장**(`azd ai agent ...` 명령 제공)을 추가합니다. `azd ai agent`를 처음 실행하면 자동 설치되기도 합니다.
...
각 lab의 `solution/` 폴더에는 `.env.example`이 있습니다. **코드를 실행할 폴더에서** 복사해 본인 값으로 채우세요. (`load_dotenv()`는 현재 작업 폴더의 `.env`를 읽습니다.)
