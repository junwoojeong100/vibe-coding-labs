...
| **Lab 1·2 권한** | 모델 호출을 위해 프로젝트 또는 상위 범위의 **Foundry User** 역할이 필요합니다. |
| **Lab 3 — 기존 프로젝트** | 프로젝트 범위의 **Foundry Project Manager** 역할이 필요합니다. |
| **Lab 3 — 새 프로젝트** | 가장 단순한 실습 구성은 대상 리소스 그룹 범위의 **Owner** 역할입니다. 리소스 생성과 역할 할당 권한을 분리한 조직에서는 관리자 지원이 필요할 수 있습니다. |
...
> 참고: MAF 패키지 자체는 Python 3.10+를 지원하지만, hosted agent quickstart와 코드 배포
> 런타임은 Python 3.13+를 기준으로 합니다. 본 가이드는 **3.14.5**로 검증했습니다.
...
설치 후 **Foundry agents 확장**(`azd ai agent ...` 명령 제공)을 추가하고 최신 버전으로 맞춥니다.

```bash
azd extension install azure.ai.agents
azd extension upgrade azure.ai.agents
azd ai agent version               # 1.0.0-beta.4 이상
azd extension list --installed     # azure.ai.agents가 보이면 OK
azd auth login                     # azd 용 Azure 로그인
```

> 이 실습이 사용하는 공식 샘플은 `azure.ai.agents` **1.0.0-beta.4 이상**을 요구합니다.
> `microsoft.foundry` 메타 확장을 설치해도 이 확장이 함께 설치될 수 있지만, 위 명령으로 실제
> `azd ai agent` 버전을 확인하세요. Beta 단계이므로 문제가 있으면
> [공식 quickstart](https://learn.microsoft.com/azure/foundry/agents/quickstarts/quickstart-hosted-agent)를 확인하세요.
...
모든 lab은 동일한 두 개의 환경 변수를 사용합니다. Lab 3에서는 프로젝트 엔드포인트를
플랫폼이 제공하고, 모델 이름은 스캐폴드된 `azure.yaml`의 `environmentVariables`가 같은 이름으로 매핑합니다.
...
- [ ] `azd version` ≥ 1.25.3, `azd ai agent version` ≥ 1.0.0-beta.4
- [ ] `copilot --version` 정상 출력, Copilot 인증 완료
- [ ] Foundry 프로젝트 엔드포인트와 모델 배포 이름 확보
- [ ] 진행할 lab에 맞는 **Foundry User / Foundry Project Manager / Owner** 역할 보유
