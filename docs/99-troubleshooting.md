...
| `ResolutionImpossible` / `resolution-too-deep` | 각 `solution/requirements.txt`의 검증 조합을 그대로 사용합니다. `agent-framework` 메타 패키지로 바꾸거나 구성 요소 버전을 임의로 섞지 마세요. |
| 기존 `.venv`에서 `agent-framework 1.8.0 requires ...` 충돌 | 이전 메타 패키지가 남은 상태입니다. `.venv`를 새로 만드는 것이 가장 확실하며, 유지해야 한다면 `pip uninstall agent-framework` 후 requirements를 다시 설치합니다. |
| hosted 호출 시 protocol version mismatch | `azure.yaml`의 Responses container version이 `2.0.0`이고 hosting이 `1.0.0a260709`인지 확인합니다. Container protocol `1.0.0`은 deprecated이며 `2.0.0`과 호환되지 않습니다. |
...
- 이 실습의 레퍼런스 코드는 MAF core **1.11.0**,
  openai/foundry **1.10.1**, orchestrations **1.0.0**,
  **agent-framework-foundry-hosting 1.0.0a260709** 조합으로 검증되었습니다.
- 재현성을 위해 필요한 구성 요소를 개별 고정했습니다. hosting 1.0.0a260709가 런타임
  의존성 `mcp`를 직접 선언하므로 별도 항목은 필요하지 않습니다.
